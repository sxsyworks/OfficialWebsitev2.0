...
const TurnTable = (props) => {
  ...
  useEffect(() => {
    ...
    let r = width >= height ? parseInt(height * 0.24) : parseInt(width * 0.3); // 0.32
    setCircle({ cx: r + 20, cy: r + 20, r });
  }, []);
  ...
  const getInfo = () => {
    ...
    return (
      <article className={styles.info}>
        ...
        <p className={styles.desc}>{formatMessage({ id: `home.application.mobile.${area}${id}.desc` })}</p>
      </article>
    );
  };
  ...
  return (
    <div className={`${styles.right} ${styles[formatMessage({ id: 'page.css.page' })]}`} id="box" ref={boxRef}>
      <div className={styles.box} style={{ width: 2 * circle.cx + 'px', height: 2 * circle.cy + 'px' }}>
        <svg id={styles.svg} viewport="0 0 100 100" version="1.1">
